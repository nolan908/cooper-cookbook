...
export default function CollectionSidebar({ 
 ...
 className="bg-fw-teal text-white px-4 py-2 text-[9px] font-black tracking-widest rounded-lg hover:opacity-90 transition-all shadow-sm" >
 ...
 <input
 type="text" value={colForm.name}
 onChange={e => setForm({ ...colForm, name: e.target.value })}
 placeholder="NAME..." className="w-full border-4 border-fw-navy p-3 text-sm font-bold focus:outline-none focus:bg-fw-yellow transition-all" required
 />
 <textarea
 value={colForm.description}
 onChange={e => setForm({ ...colForm, description: e.target.value })}
 placeholder="DESCRIPTION / BIO..." rows={2}
 className="w-full border-4 border-fw-navy p-3 text-sm font-bold focus:outline-none focus:bg-fw-yellow transition-all" />
 </div>
 <button type="submit" className="w-full bg-fw-salmon text-white rounded-xl py-4 font-black text-[10px] tracking-widest shadow-md active:scale-95 transition-all">
 CREATE
 </button>
 </form>
 )}
 ...
 <p className="text-center py-12 text-fw-navy/20 font-black text-xs italic tracking-widest">No Collections.</p>
 ) : (
 ...
 <h3 className="font-black text-sm tracking-tighter italic leading-none mb-1">{col.name}</h3>
 <p className="text-[9px] font-bold text-fw-navy/40 tracking-widest">{col.recipeCount || 0} Items</p>
 </div>
 ...
 <p className="text-[9px] font-black text-fw-navy/20 text-center py-4 italic">Empty Collection</p>
 ) : (
 ...
 <Link to={`/recipe/${recipe.id}`} className="flex-1 text-[10px] font-black hover:text-fw-salmon leading-tight">
 {recipe.title}
 </Link>
 <button 
