...
function RecipePicker({
 ...
 <h3 className="font-black text-fw-navy text-xs tracking-[0.2em]">Choose from your Stash</h3>
 ...
 className="text-[10px] font-black tracking-widest bg-fw-teal text-white border-2 border-fw-navy shadow-[2px_2px_0px_0px_rgba(17,17,17,1)] px-5 py-2 rounded transition-all active:translate-x-[1px] active:translate-y-[1px] active:shadow-none" >
 {confirmLabel}
 </button>
 <button
 onClick={onCancel}
 className="text-[10px] font-black tracking-widest bg-white text-fw-navy border-2 border-fw-navy px-5 py-2 rounded transition-all" >
 Cancel
 </button>
 </div>
 </div>
 ...
 <input
 type="text" placeholder="Search your stashed goods..." value={search}
 onChange={(e) => setSearch(e.target.value)}
 className="w-full border-4 border-fw-navy rounded-xl px-4 py-3 text-sm mb-6 focus:outline-none focus:bg-fw-yellow transition-all font-black tracking-tight text-fw-navy placeholder:text-fw-navy/20" autoFocus
 />
 ...
 <button
 key={r.id}
 type="button" onClick={() => setSelectedId(r.id)}
 className={`text-left rounded-2xl overflow-hidden border-2 transition-all relative group ${
 selectedId === r.id
 ?"border-brand-basil ring-4 ring-brand-basil/10 bg-white scale-[0.98]" :"border-transparent bg-white hover:border-brand-basil/20 shadow-sm hover:shadow-md" }`}
 >
...
export default function CollectionsPage() {
 ...
 <div>
 <h1 className="text-6xl font-black italic text-fw-navy tracking-tighter" style={{ fontFamily: 'var(--font-funky)' }}>My Collections.</h1>
 ...
 className={`px-10 py-4 border-4 border-fw-navy shadow-[6px_6px_0px_0px_rgba(17,17,17,1)] font-black tracking-widest text-xs transition-all active:translate-x-[4px] active:translate-y-[4px] active:shadow-none ${
 ...
 {showForm && (
 <form onSubmit={handleCreate} className="bg-white border-8 border-fw-navy shadow-[10px_10px_0px_0px_rgba(17,17,17,1)] p-12 mb-20 space-y-8 animate-in fade-in slide-in-from-top-4 duration-500">
 <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
 <div>
 <label className="block text-[11px] font-black text-fw-navy/40 tracking-widest mb-4">Collection Title</label>
 <input
 type="text" value={form.name}
 onChange={e => setForm({ ...form, name: e.target.value })}
 placeholder="E.G. MIDNIGHT SNACKS" className="w-full border-4 border-fw-navy p-4 text-lg font-black focus:outline-none focus:bg-fw-yellow transition-all tracking-tight" required
 />
 </div>
 <div>
 <label className="block text-[11px] font-black text-fw-navy/40 tracking-widest mb-4">Brief Narrative</label>
 <input
 type="text" value={form.description}
 onChange={e => setForm({ ...form, description: e.target.value })}
 placeholder="WHAT'S THE STORY?" className="w-full border-4 border-fw-navy p-4 text-lg font-black focus:outline-none focus:bg-fw-yellow transition-all tracking-tight" />
 </div>
 </div>
 <div className="flex justify-end pt-4">
 <button type="submit" className="bg-fw-yellow text-fw-navy border-4 border-fw-navy shadow-[6px_6px_0px_0px_rgba(17,17,17,1)] px-12 py-5 font-black text-sm tracking-widest transition-all hover:bg-white active:shadow-none active:translate-x-[6px] active:translate-y-[6px]">
 ...
 <button onClick={() => setShowForm(true)} className="mt-8 text-fw-salmon font-black tracking-widest hover:underline">Start Your First Set</button>
 </div>
 ) : (
 ...
 <h2 className="font-black text-4xl text-fw-navy tracking-tighter leading-none italic" style={{ fontFamily: 'var(--font-funky)' }}>
 {col.name}.
 </h2>
 </div>
 ...
 <span className="text-[9px] font-black text-fw-navy/20 tracking-widest">STASH SIZE</span>
 ...
 <button onClick={() => setAddingTo(addingTo === col.id ? null : col.id)} className="bg-fw-pink text-fw-navy border-2 border-fw-navy px-5 py-2 font-black text-[10px] tracking-widest hover:bg-white transition-all shadow-[3px_3px_0px_0px_rgba(17,17,17,1)] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none">
 + ADD FROM STASH
 </button>
 <button onClick={() => handleDelete(col.id)} className="text-[10px] font-black tracking-widest text-fw-navy/20 hover:text-fw-salmon transition-colors">
 DELETE
 </button>
 </div>
 </div>
 ...
 <p className="text-fw-navy/20 font-black text-2xl tracking-tighter italic" style={{ fontFamily: 'var(--font-funky)' }}>This folder is empty.</p>
 <button onClick={() => setAddingTo(col.id)} className="mt-6 text-xs font-black text-fw-teal tracking-widest hover:underline decoration-4">Add some of your stashed recipes!</button>
 </div>
 ) : (
 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
 {col.recipes.map(recipe => (
 <div key={recipe.id} className="bg-white border-4 border-fw-navy p-4 flex items-center gap-4 hover:translate-x-[4px] hover:translate-y-[4px] hover:shadow-none shadow-[4px_4px_0px_0px_rgba(17,17,17,1)] transition-all group/item relative overflow-hidden">
 <div className="w-24 h-24 border-4 border-fw-navy bg-fw-pink overflow-hidden shrink-0 shadow-[2px_2px_0px_0px_rgba(17,17,17,1)] relative">
 {recipe.imageUrl ? (
 ...
 <p className="text-[10px] font-black text-fw-navy/40 mt-1 tracking-widest">{recipe.categoryTags || "LIMITED"}</p>
 </div>
 <button
 onClick={e => { e.stopPropagation(); handleRemoveRecipe(col.id, recipe.id); }}
 className="absolute right-2 top-2 opacity-0 group-hover/item:opacity-100 text-fw-navy/20 hover:text-fw-salmon p-2 transition-all" title="Remove" >
